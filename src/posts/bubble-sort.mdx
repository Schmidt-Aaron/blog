...
Bubble sort is a sorting algorithm that works by comparing adjacent elements and swapping them if they are in the wrong order. Due to it's simplicity, it is often used to demonstrate the concept of a sorting algorithm. However, it has little use outside of the conceptual because of it's terrible performance. It has a worst case complexity of O(n^2) and a best case complexity of O(n) if the input is already sorted.
